...
def spark_ml():
	...
	print(recommendations.where(recommendations.userId <= 5).collect())
...
def spark_mllib():
	...
	rawUserData = sc.textFile('D:/note/核心BI：商务智能与推荐系统/week4/L4/MovieLens/ratings_small.csv')
	...
	model = ALS.train(training_RDD, rank=3)
	for x in range(1,6):
		print(model.recommendProducts(x,2))

def python_als():
	X = load_movie_ratings('E:/32.Project4CV/SVD4RS/ml-1m/ratings.dat')
	model = ALS()
...
def load_movie_ratings(filename):
	f = open(filename)
	# lines = iter(f)
	col_names = ','.join(next(f)[:-1].split(',')[:-1])
...
def format_prediction(item_id, score):
	return 'item_id:%d score:%0.2f' %(item_id, score)
...
def surprise_algo():
	...
	data = Dataset.load_from_file('E:/32.Project4CV/SVD4RS/ml-1m/ratings.dat',reader=reader)
	...
	# algo = SlopeOne()
    # 协同过滤
    # algo = surprise.KNNWithMeans(k=20, min_k=1, sim_options={'user_based': True, 'verbose': 'True'}) # userCF
    # algo = surprise.KNNWithMeans(k=20, min_k=1, sim_options={'user_based': False, 'verbose':True}) # itemCF
